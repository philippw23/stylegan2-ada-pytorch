...
def load_split(split_path: Path) -> Sequence[int]:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Directory with BTXRD_resized_sorted (default: {DEFAULT_DATASET_DIR})",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> None:
    ...
    train_indices = load_split(args.split_path)
    labels = load_labels(dataset_json)

    removed, kept, missing, kept_labels = delete_non_train(
    ...
    print(f"Kept {kept} train images.")
    print(f"Removed {removed} non-train images{' (dry run)' if args.dry_run else ''}.")
    if missing:
